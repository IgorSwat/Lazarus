...
namespace testing {
    ...
    // We store all tests inside a global test vector
    // - Using a dynamic structure such as vector allows to add tests dynamically, without specyfing exact number of tests
    // - We use a getter function with a static variable to ensure only one test vector, created at the header file level
    // - It's not recommended to use this function outside the tests module implementation
    inline std::vector<Test>& __get_tests()
    {
    ...
    // This function allows for compile-time creation and aggregation of tests
    #define REGISTER_TEST(name) \
        bool name(); \
        struct name##_t { \
            name##_t() { __get_tests().emplace_back(#name, name); } \
        }; \
    ...
    // - The only recommended way to access and run tests
    bool run_tests();
