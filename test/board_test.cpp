...
namespace testing {
    ...
    REGISTER_TEST(board_static_load_test)
    {
        ...
        std::vector<std::pair<std::string, Board>> cases = {
            std::make_pair("FEN", Board()), 
            std::make_pair("COPY", Board())
        };
        ...
        for (const auto& tcase : cases) {
            ...
            ASSERT_EQUALS(true, board.is_castle_path_clear(WHITE, KINGSIDE_CASTLE));
            ASSERT_EQUALS(false, board.is_castle_path_clear(WHITE, QUEENSIDE_CASTLE));
            ASSERT_EQUALS(false, board.is_castle_path_clear(BLACK, KINGSIDE_CASTLE));
            ASSERT_EQUALS(false, board.is_castle_path_clear(BLACK, QUEENSIDE_CASTLE));

            ASSERT_EQUALS(SQ_F5, board.ep_square());

            ASSERT_EQUALS(0, board.hf_clock());
    ...
    REGISTER_TEST(board_dynamic_change_test_1)
    {
        ...
        for (auto it = progress.begin(); it != progress.end(); it++) {
            ...
            ASSERT_EQUALS(reference_board.hf_clock(), test_board.hf_clock());
    ...
    REGISTER_TEST(board_repetition_count_test)
    {
        ...
        ASSERT_EQUALS(1, board.repetitions().count);
        ASSERT_EQUALS(0, board.repetitions().gap);
        ...
        ASSERT_EQUALS(2, board.repetitions().count);
        ASSERT_EQUALS(4, board.repetitions().gap);
    ...
    // TODO: uncomment after adding threat calculation
